...
event_name = 'registration_succeeded'
...
def get_platform_events(platform):
    ...
    return event_data_df.alias("ed")\
            .withColumn("month",
                     F.format_string("%02d",F.month(F.col("ed.timestamp"))))\
            .withColumn("day",
                     F.format_string("%02d",F.dayofmonth(F.col("ed.timestamp"))))\
            .selectExpr("ed.id as event_id",
                    "ed.platform as platform",
                    "ed.anonymous_id as anonymous_id",
                    "ed.user_id as user_id",
                    "ed.timestamp as event_timestamp",
...
## Create Roll-ups
backend_cleaned = get_platform_events('backend')

## Union platforms into single dataframe
all_records = backend_cleaned
