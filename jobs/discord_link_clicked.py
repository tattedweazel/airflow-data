...
event_name = 'discord_link_clicked'
...
def get_platform_events(platform):
    ...
    if 'member_tier' not in event_data_df.columns:
        event_data_df = event_data_df.withColumn('member_tier', F.lit(None).cast("string"))
    ...
    return event_data_df.alias("ed") \
        ...
        .withColumn('member_tier',
                    F.when((F.col("ed.member_tier").isNull()) & (F.col("ed.user_id").isNull()), "anon")
                    .when((F.col("ed.member_tier").isNull()) & (F.col("ed.user_id").isNotNull()), "free")
                    .when((F.lower(F.col("ed.member_tier")) == 'free') & (F.col("ed.user_id").isNull()), "anon")
                    .otherwise(F.lower(F.col("ed.member_tier")))) \
        .selectExpr("ed.id as event_id",
                    "ed.user_id as user_id",
                    "member_tier as user_tier",
