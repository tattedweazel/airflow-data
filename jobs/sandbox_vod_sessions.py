...
def get_heartbeat_aggregates(heartbeat_data, platform):
    ...
    if 'context_page_referrer' not in heartbeat_data.columns:
        heartbeat_data = heartbeat_data.withColumn('context_page_referrer', F.lit(None).cast("string"))

    if 'context_ip' not in heartbeat_data.columns:
        heartbeat_data = heartbeat_data.withColumn('context_ip', F.lit(None).cast("string"))

    if 'context_locale' not in heartbeat_data.columns:
        heartbeat_data = heartbeat_data.withColumn('context_locale', F.lit(None).cast("string"))


    return heartbeat_data.groupby('session_id', 'content_id', 'anonymous_id', 'user_tier', uuid_column, 'context_ip', 'context_locale', 'context_page_referrer').count()
...
def get_platform_aggregate(heartbeat_base, counts_by_session, session_starts, session_ends, max_positions, dim_users, platform):
    ...
    return heartbeat_base.alias("hb").\
        ...
        .selectExpr("CONCAT(hb.session_id, CONCAT(year(ss.start_timestamp), CONCAT(month, day))) as session_id",
                    ...
                    "hb.context_ip as context_ip",
                    "hb.context_locale as context_locale",
                    "substring(hb.context_page_referrer, 1, 512) as context_page_referrer",
...
if all_records:
    write_to_s3(all_records, 'sandbox_vod_sessions')
