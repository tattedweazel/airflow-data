...
def load_event_data(platform):
        ...
        elif platform == 'chat_js':
            segment_source = 'itvYWmS95YQ7hGnm2TErbe'
        else:
...
def get_platform_events(platform):
        ...
        if 'target' not in event_data_df.columns:
            event_data_df = event_data_df.withColumn('target', F.lit(None).cast("string"))
        if 'room_id' not in event_data_df.columns:
            event_data_df = event_data_df.withColumn('room_id', F.lit(None).cast("string"))
        ...
        return event_data_df.alias("ed")\
                ...
                .selectExpr("ed.id as chat_event_id",
                        "ed.actor as actor",
                        "ed.action as action",
                        "ed.target as target",
                        "ed.room_id as room_id",
                        ...
                        "month",
                        "day")\
                .dropDuplicates(subset=['chat_event_id'])
...
chat_js_cleaned = get_platform_events('chat_js')

## Union platforms into single dataframe
all_records = chat_js_cleaned
