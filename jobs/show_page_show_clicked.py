...
event_name = 'show_page_show_clicked'
...
def get_platform_events(platform):
    ...
    if 'filter_channel_id' not in event_data_df.columns:
        event_data_df = event_data_df.withColumn("filter_channel_id", F.lit(None).cast("string"))

    if 'filter_genre' not in event_data_df.columns:
        event_data_df = event_data_df.withColumn("filter_channel_id", F.lit(None).cast("string"))
    ...
    return event_data_df.alias("ed") \
        ...
        .selectExpr("ed.id as event_id",
                    ...
                    "user_tier",
                    "ed.show_id as series_id",
                    "ed.sort as sort",
                    "ed.filter_channel_id as filter_channel_id",
                    "ed.filter_genre as filter_genre",
                    "ed.on_mobile_device as on_mobile_device",
