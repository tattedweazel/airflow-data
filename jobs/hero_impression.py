...
def get_platform_events(platform):
    ...
    if 'platform' not in event_data_df.columns:
        event_data_df = event_data_df.withColumn('platform', F.lit(platform_lower).cast("string"))
    ...
    return event_data_df.alias("ed") \
        ...
        .selectExpr("ed.id as event_id",
                    "ed.context_page_path as location",
...
## Create Roll-ups
web_cleaned = get_platform_events('Web')  # Web only event for now
