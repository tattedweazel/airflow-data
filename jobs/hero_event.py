...
def get_platform_events(platform):
    ...
    if 'has_sso' not in event_data_df.columns:
        event_data_df = event_data_df.withColumn('has_sso', F.lit('false').cast(BooleanType()))

    if 'has_video' not in event_data_df.columns:
        event_data_df = event_data_df.withColumn('has_video', F.lit('false').cast(BooleanType()))
