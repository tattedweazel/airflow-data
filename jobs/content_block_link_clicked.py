...
event_name = 'content_block_link_clicked'
...
def get_platform_events(platform):
        ...
        if 'content_block_item_use_type' not in event_data_df.columns:
            event_data_df = event_data_df.withColumn('content_block_item_use_type', F.lit(None).cast("string"))

        if 'static_spotlight_element_clicked' not in event_data_df.columns:
            event_data_df = event_data_df.withColumn('static_spotlight_element_clicked', F.lit(None).cast("string"))

        if 'static_spotlight_element_copy' not in event_data_df.columns:
            event_data_df = event_data_df.withColumn('static_spotlight_element_copy', F.lit(None).cast("string"))
        ...
        return event_data_df.alias("ed")\
                ...
                .selectExpr("ed.id as message_id",
                        ...
                        "ed.user_tier as user_tier",
                        "ed.content_block_display_type as content_block_display_type",
                        "ed.content_block_item_use_type as content_block_item_use_type",
                        "ed.content_block_uuid as content_block_uuid",
                        "ed.location as location",
                        "ed.target_url as target_url",
                        "ed.received_at as event_timestamp",
                        f"'{platform_lower}' as platform",
                        "ed.static_spotlight_element_clicked",
                        "ed.static_spotlight_element_copy",
