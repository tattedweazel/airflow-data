...
event_name = 'series_rss_signup_successful'
...
def get_platform_events(platform):
    ...
    if 'series_uuid' not in event_data_df.columns:
        event_data_df = event_data_df.withColumn('series_uuid', F.lit(None).cast("string"))
...
all_records = web_cleaned  # Only on web
