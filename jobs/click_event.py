...
def get_platform_events(platform):
    ...
    if 'context' not in event_data_df.columns:
        event_data_df = event_data_df.withColumn('context', F.lit(None).cast("string"))
    ...
    return event_data_df.alias("ed") \
        ...
        .selectExpr("ed.id as event_id",
                    ...
                    "substr(ed.label, 1, 256) as label",
                    "ed.platform as platform",
                    "substr(ed.context, 1, 256) as context",
                    "ed.on_mobile_device",
