...
bucket_name = 'template_dag'
staging_table = 'example1'
target_table = 'example1'
...
dag_builder = DagBuilder(bucket_name)
...
def copy_wrapper(run_date):
    ...
    task = S3ToRedshiftOperator(
        ...
        s3_key=f"{S3_COPY_SOURCE}/{bucket_name}/year={run_date['year']}/month={run_date['month']}/day={run_date['day']}/",
        table=f'stage_{staging_table}',
...
stage_copy = StageCopyOperator(
    ...
    dag=dag,
    stage_table=f'stage_{staging_table}',
