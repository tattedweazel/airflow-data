...
dag_name = 'community_signup_backfill_dag'
schedule = None
...
def truncate_table_wrapper(run_date, target_event):
    clean_date = f"{run_date['year']}-{run_date['month']}-{run_date['day']}"
    task = TruncateTableOperator(
        task_id=f'stage_truncate_{target_event}_{clean_date}',
        dag=dag,
...
for event_name, data in EVENTS.items():
    ...
    for run_date in RUN_DATES:
        truncate_table_tasks.append(truncate_table_wrapper(run_date, event_name))
